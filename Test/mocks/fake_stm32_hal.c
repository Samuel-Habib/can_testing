 ...
 * ========================================================================= */
DEFINE_FAKE_VOID_FUNC3(HAL_GPIO_WritePin, GPIO_TypeDef*, uint16_t, GPIO_PinState);
DEFINE_FAKE_VOID_FUNC2(HAL_GPIO_TogglePin, GPIO_TypeDef*, uint16_t);
DEFINE_FAKE_VALUE_FUNC2(GPIO_PinState, HAL_GPIO_ReadPin, GPIO_TypeDef*, uint16_t);
DEFINE_FAKE_VOID_FUNC2(HAL_GPIO_Init, GPIO_TypeDef*, const GPIO_InitTypeDef*);

DEFINE_FAKE_VALUE_FUNC4(HAL_StatusTypeDef, HAL_UART_Transmit, UART_HandleTypeDef*, const uint8_t*, uint16_t, uint32_t);
DEFINE_FAKE_VALUE_FUNC4(HAL_StatusTypeDef, HAL_UART_Receive, UART_HandleTypeDef*, uint8_t*, uint16_t, uint32_t);

DEFINE_FAKE_VALUE_FUNC1(HAL_StatusTypeDef, HAL_IWDG_Refresh, IWDG_HandleTypeDef*);

DEFINE_FAKE_VALUE_FUNC3(HAL_StatusTypeDef, HAL_FDCAN_AddMessageToTxFifoQ, FDCAN_HandleTypeDef*, const FDCAN_TxHeaderTypeDef*, const uint8_t*);
DEFINE_FAKE_VALUE_FUNC2(uint32_t, HAL_FDCAN_GetRxFifoFillLevel, FDCAN_HandleTypeDef*, uint32_t);
DEFINE_FAKE_VALUE_FUNC4(HAL_StatusTypeDef, HAL_FDCAN_GetRxMessage, FDCAN_HandleTypeDef*, uint32_t, FDCAN_RxHeaderTypeDef*, uint8_t*);
DEFINE_FAKE_VALUE_FUNC3(HAL_StatusTypeDef, HAL_FDCAN_ConfigInterruptLines, FDCAN_HandleTypeDef*, uint32_t, uint32_t);
DEFINE_FAKE_VOID_FUNC2(HAL_FDCAN_RxFifo0Callback, FDCAN_HandleTypeDef*, uint32_t);
...
DEFINE_FAKE_VALUE_FUNC3(BaseType_t, xQueueSend, QueueHandle_t, const void*, TickType_t);
DEFINE_FAKE_VALUE_FUNC3(BaseType_t, xQueueSendToBack, QueueHandle_t, const void*, TickType_t);
DEFINE_FAKE_VALUE_FUNC3(BaseType_t, xQueueReceive, QueueHandle_t, void*, TickType_t);
DEFINE_FAKE_VALUE_FUNC1(BaseType_t, xQueueReset, QueueHandle_t);
DEFINE_FAKE_VALUE_FUNC1(UBaseType_t, uxQueueMessagesWaiting, const QueueHandle_t);

DEFINE_FAKE_VOID_FUNC0(taskENTER_CRITICAL);
DEFINE_FAKE_VOID_FUNC0(taskEXIT_CRITICAL);
DEFINE_FAKE_VALUE_FUNC4(BaseType_t, xTaskNotifyWait, uint32_t, uint32_t, uint32_t*, TickType_t);
